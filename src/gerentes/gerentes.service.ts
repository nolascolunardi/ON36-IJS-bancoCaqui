...
export class GerentesService {
    ...
    private writeContas(contas: Conta[]): void{
        fs.writeFileSync(this.filePathConta, JSON.stringify(contas, null, 2), 'utf-8');
    }

    // requisitos relacionado a clientes
    createCliente(nome: string, cpf: string, email: string, telefone: string, dataNasc: string, endereco: string): Cliente {

        const usuarios = this.readClientes();
        const newCliente = {
            idCliente: usuarios.length > 0 ? usuarios[usuarios.length - 1].idCliente + 1 : 1,
            nome,
            cpf,
            email,
            telefone,
    ...
    removeCliente(id: number): void{
        ...
        const index = clientes.findIndex(cliente => cliente.idCliente === Number(id));
    ...
    createConta(idCliente: number, numeroConta: string, tipoConta: TipoConta, saldo: number ): Conta {
    ...
    //     const contas = this.readContas();
    //     const conta = contas.find(conta => conta.idCliente === Number(id));
    ...
    findByIdCliente(id: number): Cliente{
        const clientes = this.readClientes();
        const cliente = clientes.find(cliente => cliente.idCliente === Number(id));
